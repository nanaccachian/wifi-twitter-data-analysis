...
INSERT INTO dbo.dim_datetimes
	...
	WHERE 
		...
		NOT IN (SELECT LTRIM(STR(ti_day))+LTRIM(STR(ti_month))+LTRIM(STR(ti_day))+CONVERT(NVARCHAR(8),ti_time) FROM dbo.dim_datetimes)
...
INSERT INTO dbo.fact_tweets (tw_id,tw_created_at,tw_text,tw_user)
	SELECT
		tw_id,
		(SELECT ti_id FROM dbo.dim_datetimes 
