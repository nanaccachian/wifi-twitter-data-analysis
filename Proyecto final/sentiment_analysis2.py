...
df_tweets['severity'] = df_tweets.apply(lambda row: sia.sentiment(str(row.tw_text)), axis=1)
df_replies['severity'] = df_replies.apply(lambda row: sia.sentiment(str(row.rp_text)), axis=1)
df_quotes['severity'] = df_quotes.apply(lambda row: sia.sentiment(str(row.qu_text)), axis=1)
...
index_names = df_severity[df_severity['severity'] >= 0.5].index
